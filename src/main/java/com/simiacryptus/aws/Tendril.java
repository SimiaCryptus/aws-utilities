...
public class Tendril {
    ...
    public static TendrilLink getControl(final int localControlPort) {
        return getControl(localControlPort, 10);
    }
     ...
     */
    public static class TendrilControl implements AutoCloseable {
         ...
         */
        public TendrilControl(final TendrilLink inner) {
            this.inner = inner;
        }
    ...
    protected static class TendrilLinkImpl implements TendrilLink {
        ...
        public void exit(final int status, final int wait) {
            contacted = true;
            logger.warn(String.format("Exiting with eval %d in %d", status, wait));
            new Thread(() -> {
                try {
                    Thread.sleep(wait);
                } catch (InterruptedException e) {
